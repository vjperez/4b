...
while($index < 3){
if ($dbQueries[$index] != ''){
  if( ! $entradasarray = mysqli_query($cxn, $dbQueries[$index]) ){
    $mensaje1 = 'Error loading entries.';
    $mensaje2 = $dbQueries[$index] . '<br>' . 'No tengo arreglo de entradas extras. (Ni vacio!)' . '<br>' . HOST_FS_ROOT . 'escritos/php/sacalo/sacalo.php: ' . mysqli_error($cxn);    
    mysqli_close($cxn);  
